...
export class Terminal extends Component {
    constructor() {
        ...
        this.child_directories = {
            ...
            books: [
                "The_Book_Thief.pdf",
                "The_Priory_of_the_Orange_Tree.pdf",
                "Brida.pdf"
            ],
            skills: [
                "React.js",
    ...
    reStartTerminal = () => {
        ...
        this.setState({ terminal: [] }, () => {
            // initial welcome line(s)
            const terminal = this.state.terminal;
            terminal.push(
                <div key={"welcome-line"} className="mb-2">
                    <div className="text-sm font-normal text-gray-300">Welcome to <span className="font-bold">AanandiOS Terminal</span> — type <span className="text-ubt-blue">help</span> to see available commands.</div>
                </div>
            );
            this.setState({ terminal }, () => {
        ...
        return (
            <React.Fragment key={id}>
                <div className="flex w-full h-5">
                    <div className="flex">
                        <div className=" text-ubt-green">aanandi@AanandiOS</div>
    ...
    focusCursor = (e) => {
        clearInterval(this.cursor);
        // If clicking the container instead of input element, find the row id
        let rowId = $(e.target).data("row-id");
        if (!rowId) {
            rowId = $(e.currentTarget).find('input').data('row-id');
        }
    ...
    handleCommands = (command, rowId) => {
        ...
        // a small helper to return the standard "available commands" text
        const availableCommands = () => {
            return "Available Commands: [ cd, ls, pwd, echo, clear, exit, mkdir, code, spotify, chrome, about-aanandi, todoist, trash, settings, sendmsg ]";
        }
        ...
        switch (main) {
            ...
            case "pwd":
                ...
                result = str.replace("~", "/home/aanandi")
                break;
            case "code":
                if (words[0] === "." || words.length === 0) {
                    this.props.openApp("vscode");
                } else {
                    result = "Command '" + main + "' not found, or not yet implemented.<br>" + availableCommands();
                }
                break;
            case "echo":
            ...
            case "about-aanandi":
                if (words[0] === "." || words.length === 0) {
                    this.props.openApp("about-aanandi");
                } else {
                    result = "Command '" + main + "' not found, or not yet implemented.<br>" + availableCommands();
                }
                break;
            ...
            case "sendmsg":
                ...
                break;
            case "help":
                result = "<div class='text-sm'>" +
                    "<b>Available commands:</b><br/>" +
                    "cd &nbsp;&nbsp; ls &nbsp;&nbsp; pwd &nbsp;&nbsp; echo &nbsp;&nbsp; clear &nbsp;&nbsp; exit &nbsp;&nbsp; mkdir <br/>" +
                    "code &nbsp;&nbsp; spotify &nbsp;&nbsp; chrome &nbsp;&nbsp; about-aanandi &nbsp;&nbsp; todoist &nbsp;&nbsp; trash <br/>" +
                    "settings &nbsp;&nbsp; sendmsg" +
                    "</div>";
                break;
            case "clear":
            ...
            case "sudo":
                ...
                ReactGA.event({
                    ...
                    action: "attempted-sudo",
                });

                result = "<img class=' w-2/5' src='./images/memes/used-sudo-command.webp' />" + "<div class='mt-2 text-sm'>Nice try 😉. No privileges elevated here.</div>";
                break;
            default:
                result = "Command '" + main + "' not found, or not yet implemented.<br>" + availableCommands();
        }
